...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['send_message'])) {
    ...
    $content = sanitizeInput($_POST['content']);
            ...
            $stmt->bind_param("iiss", $admin_id, $recipient_id, $subject, $content);
...
if ($message_id && $action == 'view') {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iiiiii", $admin_id, $admin_id, $message_id, $admin_id, $admin_id, $admin_id);
    $stmt->execute();
...
$sql = "SELECT id, name, user_type FROM users WHERE id != ? AND user_type IN ('teacher', 'student') ORDER BY name";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $admin_id);
$stmt->execute();
...
<body class="bg-gray-100">
    <?php renderNavigation('admin', 'messages'); ?>
        ...
        <div class="px-4 py-6 sm:px-0">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-gray-600">Send and receive messages with teachers and students</p>
                </div>
                <?php if ($action == 'list'): ?>
                    <a href="?action=compose" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
                            ...
                            <select name="recipient_id" id="recipient_id" required
                                ...
                                <?php while ($user = $users->fetch_assoc()): ?>
                                    <option value="<?php echo $user['id']; ?>">
                                        <?php echo htmlspecialchars($user['name']); ?> (<?php echo ucfirst($user['user_type']); ?>)
                                    </option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-gray-600 mt-1">
                                <?php echo $message['sender_id'] == $admin_id ? 'To: ' : 'From: '; ?>
                            ...
                            <?php while ($msg = $received_messages->fetch_assoc()): ?>
                                <div class="px-6 py-4 hover:bg-gray-50">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="ml-4">
                                            <a href="?action=view&id=<?php echo $msg['id']; ?>" class="text-indigo-600 hover:text-indigo-900">
