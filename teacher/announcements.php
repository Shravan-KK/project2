...
$page_title = 'Announcements - Teacher';
...
$sql = "SELECT a.*, u.name as created_by_name,
        ...
        WHERE a.target_audience IN ('teachers', 'both') AND a.is_active = 1
...
if ($announcement_id && $action == 'view') {
    $sql = "SELECT a.*, u.name as created_by_name
            FROM announcements a 
            LEFT JOIN users u ON a.created_by = u.id
            WHERE a.id = ? AND a.target_audience IN ('teachers', 'both') AND a.is_active = 1";
    $stmt = $conn->prepare($sql);
                        ...
                        <?php while ($announcement = $announcements->fetch_assoc()): ?>
                            <div class="px-6 py-4 hover:bg-gray-50">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="ml-4">
                                        <a href="?action=view&id=<?php echo $announcement['id']; ?>" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-green-600 hover:bg-green-700">
